...
// Standalone (non-row) RTMP server settings page. Used both:
//  - As the destination of the row inside IngestsSettingsView
//    (via RtmpServerSettingsView).
//  - As a direct top-level entry in SettingsView so the user reaches
//    the page in one tap instead of going through Ingests.
struct RtmpServerStandaloneSettingsView: View {
    @EnvironmentObject var model: Model
    @ObservedObject var rtmpServer: SettingsRtmpServer

    private func submitPort(value: String) {
        guard let port = UInt16(value) else { return }
