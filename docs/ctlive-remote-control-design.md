...
### 11.2.1 行駛實測（2026-08-16）

在真實行動網路、車輛行駛狀態下（最高 69 km/h，市區與快速路段）
**連續 40 分鐘零斷線、零重連**。裝置持續回報 `topRight.location`，可獨立佐證確實在移動。

**這筆資料排除了什麼**：

- ✅ 「行駛中基地台交接會頻繁打斷控制連線」—— 不成立
- ✅ 5s/15s 的應用層 ping + assistant 的 pong 足以維持連線存活
- ✅ 同日稍早那幾次 156 / 170 / 189 秒的斷線**不是行駛中的常態**

**這筆資料沒有排除什麼** —— 這一點更重要，因為很容易被誤讀：

- ❌ **「斷線後能否自動恢復」完全沒有被驗證。** 沒有斷線就沒有恢復可測。
- 「40 分鐘零斷線」與「弱網自動恢復可用」是**兩件無關的事**。
  不要用前者去支持後者。

弱網自動恢復仍是**設計已完成、實地待驗**：需要一次夠深的訊號中斷
（隧道、地下停車場、基地台邊緣）才能驗，而且要在兩邊都不操作的條件下發生。
截至 2026-08-16 為止**零次乾淨驗證** —— 期間每一次看似自動重連的事件，
附近都有人為操作（後端重啟容器、重裝 App、擷取程序終止 App）。
...
### 12.-1 定位權限必須是「永遠」，否則 App 活不過十分鐘

**這是目前唯一會讓一台裝置在賽事中安靜消失、且完全看不出原因的設定。**

`Location.swift` 原本只呼叫 `requestWhenInUseAuthorization()`，**從來不要求「永遠」**。
全新安裝拿到的必然是「使用 App 期間」，除非有人事後手動改。造成的因果鏈：

```
權限=使用 App 期間
  → App 進背景 → 定位停止 → 背景執行權消失
    → iOS 暫停 → 記憶體壓力下終止
      → 沒有進行中的背景模式 → iOS 沒有理由重新拉起
```

**2026-08-17 實測，同一台全新安裝的裝置、自動鎖定已關、無人操作：**
...
兩次的死亡都是用 `devicectl device info processes` **直接觀測**到的，不是從連線時長推論
（`alive=` 是**連線**存活時間，不是行程存活時間，兩者不可混用）。

**7 分鐘短到讓「人工中途檢查」這個備援也失效** —— 沒有人能每七分鐘巡一次六台手機。

### 修正（三項缺一不可）

1. **主動要求「永遠」**。iOS 不允許一步到位，必須先取得 WhenInUse 再要求升級，
   所以授權變更的回呼裡也要接一次
2. **`INFOPLIST_KEY_NSLocationAlwaysAndWhenInUseUsageDescription`**。
   原本沒有這個鍵，**iOS 會靜默忽略 Always 請求** —— 缺了它，第 1 項完全無效
3. **App 內明講**。CTLive 設定頁在權限不足時顯示紅字警告，
   並在 status 的 `general.canRunInBackground` 回報給儀表板

第 3 項的理由：**這個設定從外面完全看不出來**，而它決定一台機器能不能不靠人走過去就救回來。
六台手機在起跑線上，正是沒有人會逐台檢查的時候。

⚠️ **權限對話框只有人能按，這是整條鏈唯一無法自動化的一步。** 賽前檢查必須包含它。

### 12.0 最高風險：App 被終止後，控制連線不會自己回來

**這是目前已知最嚴重的問題，比弱網和電量都嚴重。**

`Model.setup()`（建立控制連線、啟動位置更新的地方）**只從 `MainView.onAppear` 呼叫**。
而 `Info.plist` 的 `UIBackgroundModes` 含 `location`，所以 iOS 會在記憶體壓力下終止 App、
之後再於背景把它拉起來 —— **背景啟動時沒有 scene 出現，`onAppear` 不觸發，`setup()` 不跑**。

形成一個閉環的失敗：

```
iOS 因記憶體壓力終止 App
  → iOS 透過 location 背景模式重新拉起
    → 沒有 scene → setup() 不跑
      → 控制連線不建立（導播失去這台機器）
      → 位置更新也不重啟（而那正是讓 App 在背景活著的東西）
        → iOS 過一會兒又把它睡掉
```

**2026-08-16 實測**：行駛中連續執行 65 分鐘後 App 被 iOS 終止
（`code=1006 alive=3908s`，電量 34% 且充電中，排除電量因素）。
iOS 一分鐘內把行程拉起來（PID 改變可證），但後端**在那之後 7.4 分鐘零握手** ——
遠超退避序列的總長（累積約 4 分鐘），代表它根本沒有開始嘗試連線。

**現場的意義**：手機在攝影師口袋裡、螢幕關著、人在拍攝中。
沒有人會去打開 App，所以導播是**永久**失去那台機器，而且現場不會有人察覺。

**修正**：`AppDelegate.didFinishLaunchingWithOptions` 偵測 `applicationState == .background`，
只拉起控制連線與位置更新（相機與媒體管線刻意不碰）。

⚠️ **此修正 UNVERIFIED**，兩個理由：
1. 重現背景重啟需要實機，尚未做過
2. 它假設 `globalModel` 在 `didFinishLaunching` 時已建好，這點未經實證。
   該分支刻意記 log 而非靜默失敗，測試時看得出假設是否成立

**在真的觀察到「背景重啟後自動連上」之前，不可視為已解決。**
在那之前，長時間賽事務必安排中途人工確認裝置是否仍在線。
